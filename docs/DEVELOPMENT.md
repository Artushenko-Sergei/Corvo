# Corvo — документация для разработчиков

Технические подробности: сборка, упаковка, архитектура, поведение на X11/Wayland.
Если вы хотите просто пользоваться программой — см. [README](../README.md).

| | |
|---|---|
| Версия | см. `CMakeLists.txt` (`project(... VERSION)` + `CORVO_BUILD`) |
| Лицензия | MIT |
...
| Пакет | CPack → `Corvo_2.0.1+30_amd64.deb` (локальный: Qt подключается через RPATH), плюс каталог `debian/` |
...
```
MAJOR . MINOR . PATCH + BUILD          например: 2.0.1+30
```
...
```bash
scripts/bump-version.sh build  "поправлена ошибка в разборе заголовка"   # 2.0.1+29 → 2.0.1+30
scripts/bump-version.sh patch  "исправлено падение при обрыве сети"      # 2.0.1+29 → 2.0.2+30
scripts/bump-version.sh minor  "добавлен экспорт чатов"                  # 2.0.1+29 → 2.1.0+30
scripts/bump-version.sh major  "новый формат каталога профиля"           # 2.0.1+29 → 3.0.0+30
```
...
│   └── screenshots/            # для README и AppStream
│
├── src/
...
./build/Corvo --version     # Corvo 2.0.1+30
./build/Corvo --help
```
...
cd build
cpack -G DEB
sudo apt install ./Corvo_2.0.1+30_amd64.deb
```
...
```
/usr/bin/Corvo
/usr/share/applications/io.github.artushenko_sergei.Corvo.desktop
/usr/share/metainfo/io.github.artushenko_sergei.Corvo.metainfo.xml
/usr/share/icons/hicolor/{16x16,…,256x256}/apps/io.github.artushenko_sergei.Corvo.png
/usr/share/icons/hicolor/scalable/apps/io.github.artushenko_sergei.Corvo.svg
/usr/share/pixmaps/io.github.artushenko_sergei.Corvo.png
...
Проверяется так — бинарник из пакета запускается без своего RPATH:

```bash
dpkg-deb -x build/Corvo_2.0.1+30_amd64.deb /tmp/t
/lib64/ld-linux-x86-64.so.2 --inhibit-rpath '' /tmp/t/usr/bin/Corvo --version
# libQt6Core.so.6: version `Qt_6.11' not found
```
...
Чтобы раздавать другим, Qt нужно доставить вместе с приложением:
...
Qt ≥ 6.11 есть в репозитории (Debian trixie/sid).
...
cd build && cpack -G DEB                               # ✓ Corvo_2.0.1+30_amd64.deb
```
